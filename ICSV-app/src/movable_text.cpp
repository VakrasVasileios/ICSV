 ...
 * @update	2012 to work with newer versions of OGRE by MindCalamity
 *<mindcalamity@gmail.com>
 *	- See "Notes" on:
...
void
MovableText::setCaption(const String& caption) {
  if (caption != mCaption) {
    mCaption    = caption;
    mNeedUpdate = true;
  }
...
void
MovableText::setColor(const ColourValue& color) {
  if (color != mColor) {
    mColor        = color;
    mUpdateColors = true;
  }
...
void
MovableText::setCharacterHeight(Real height) {
...
void
MovableText::setSpaceWidth(Real width) {
...
void
MovableText::setTextAlignment(const HorizontalAlignment& horizontalAlignment,
...
void
MovableText::setGlobalTranslation(Vector3 trans) {
  mGlobalTranslation = trans;
}

void
MovableText::setLocalTranslation(Vector3 trans) {
...
MovableText::_setupGeometry() {
  ...
  if (!decl->findElementBySemantic(VES_TEXTURE_COORDINATES))
    decl->addElement(POS_TEX_BINDING,
                     offset,
                     Ogre::VET_FLOAT2,
    ...
    if (newLine) {
      ...
      for (String::iterator j = i; j != iend && *j != '\n'; j++) {
        if (*j == ' ')
          ...
          len += mpFont->getGlyphAspectRatio((unsigned char) *j) * mCharHeight
              * 2.0;
      }
      newLine = false;
    }
...
const Quaternion&
MovableText::getWorldOrientation(void) const {
  assert(mpCam);
...
void
MovableText::getWorldTransforms(Matrix4* xform) const {
  if (this->isVisible() && mpCam) {
    ...
    Vector3 ppos = mParentNode->_getDerivedPosition()
        + Vector3::UNIT_Y * mGlobalTranslation;
...
void
MovableText::getRenderOperation(RenderOperation& op) {
  if (this->isVisible()) {
    if (mNeedUpdate)
      this->_setupGeometry();
    if (mUpdateColors)
...
void
MovableText::_notifyCurrentCamera(Camera* cam) {
  mpCam = cam;
}

void
MovableText::_updateRenderQueue(RenderQueue* queue) {
  if (this->isVisible()) {
    if (mNeedUpdate)
      this->_setupGeometry();
    if (mUpdateColors)
